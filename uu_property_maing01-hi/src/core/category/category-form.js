...
export const LocationFrom = createVisualComponent({
  ...
  render(props) {
    const { category, handlerMap, onClose } = props;
    //@@viewOn:hooks
    ...
    function handleOnSubmitClick({ data }) {
      if (category) {
        handlerMap
          .update({ ...data.value, id: category.id })
          .then(() => {
            addAlert({
    ...
    return (
      <Form onSubmit={handleOnSubmitClick}>
        <div className={CLASS_NAMES.grid()}>
          <FormText name="name" label={<Lsi lsi={LsiData.name} />} required initialValue={category && category.name} />
          <FormText
            name="description"
        ...
        <div className={CLASS_NAMES.controls()}>
          <SubmitButton style={{ marginRight: "8px" }} lsi={{ submit: category ? LsiData.update : LsiData.create }} />
